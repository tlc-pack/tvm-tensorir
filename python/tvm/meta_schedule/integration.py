...
@register_object("meta_schedule.IntegrationContext")
class IntegrationContext(Object):
    ...
    def query(
        ...
        """
        return _ffi_api.IntegrationContextQuery(  # type: ignore # pylint: disable=no-member
            self,
            task_name,
            mod,
            dispatched,
    ...
    @staticmethod
    def current() -> Optional["IntegrationContext"]:
        """The context manager in the current scope

        Returns
        -------
        ctx : Optional[IntegrationContext]
            The IntegrationContext in the current scope.
            NullOpt if it's currently not under any IntegrationContext.
        """
        return _ffi_api.IntegrationContextCurrent()  # type: ignore # pylint: disable=no-member
    ...
    def entry_point(
        ...
        result : Union[IRModule, RelayFunc, PrimFunc, None]
            ...
            4) IRModule for unified dispatch
        """
        return _ffi_api.IntegrationContextEntryPoint(  # type: ignore # pylint: disable=no-member
            task_name,
            mod,
            dispatched,
        )
    ...
    def __enter__(self) -> "IntegrationContext":
        ...
        _ffi_api.IntegrationContextEnterScope(self)  # type: ignore # pylint: disable=no-member
        return self

    def __exit__(self, ptype, value, trace) -> None:
        """Exiting the scope of the context manager"""
        _ffi_api.IntegrationContextExitScope(self)  # type: ignore # pylint: disable=no-member


@register_object("meta_schedule.TaskExtraction")
class TaskExtraction(IntegrationContext):
...
class ApplyHistoryBest(IntegrationContext):
    pass
