...
class StmtSRef(Object):
    ...
    - Sref tree: The parent-children-relationship of srefs form a tree,
...
class BlockScope(Object):
    ...
    - Dominant block: a block who is the only writer of its output, which dominates
    the reader of its output buffers
    - Complete block: a block that satisfies the following conditions:
        1) a dominant block: the block is the only writer of its output,
        which dominates the reader of its output buffers
        2) all block vars are data parallel
        3) no overlap between the buffers it reads and writes
    - Reduction block: a block that satisfies the following conditions:
        1) a dominant block: the block is the only writer of its output,
        which dominates the reader of its output buffers
