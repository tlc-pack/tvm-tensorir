...
class SRefMapCreator : public StmtVisitor {
 public:
  ...
  static std::unordered_map<const StmtNode*, StmtSRef> Create(const PrimFunc& func) {
    ...
    visitor(func->body);
    std::unordered_map<const StmtNode*, StmtSRef> ret = std::move(visitor.stmt2ref_);
    return ret;
  }
  /*!
  ...
  void PopSRef() {
    const StmtSRef& sref = frames_.back();
...
class ScopeMapCreator : public StmtVisitor {
 public:
  ...
  void VisitStmt_(const BlockNode* block) override {
    ...
    const auto& sref = stmt2ref_.at(block);
    scopes_[sref] = std::move(frames_.back().scope);
    // Pop stack
    frames_.pop_back();
 ...
 private:
  ...
  const std::unordered_map<const StmtNode*, StmtSRef>& stmt2ref_;
