  ...
  if (loop_sref_opt) {
    ...
    for (int i = 0, n = block->iter_vars.size(); i < n; ++i) {
      ...
      for (const StmtSRef& higher_loop : loops) {
        ...
        CHECK(!UsesVar(binding, [&loop_var](const VarNode* var) { return var == loop_var.get(); }))
    ...
    for (int i = static_cast<int>(loops.size()) - 1; i >= 0; --i) {
      ...
      for (const PrimExpr& expr : init_realize->iter_values) {
        ...
        if (!UsesVar(expr,
                     [v = higher_loop->loop_var](const VarNode* var) { return var == v.get(); })) {
          continue;
        }
...
void MergeReduction(ScheduleState self, const StmtSRef& init_sref, const StmtSRef& update_sref) {
  ...
  CHECK(init_body != nullptr && update_body != nullptr)
      << "ValueError: 'merge_reduction' expects the body of init and update block to be "
         "BufferStore";
  CommReducer reducer;
  PrimExpr reducer_lhs, reducer_rhs;
  std::tie(reducer, reducer_lhs, reducer_rhs) =
      GetReducerAndCombinerLhsRhs(self, init_body->value, GetRef<BufferStore>(update_body));
  ...
  StmtSRef scope = GetScopeRoot(self, init_sref, /*require_stage_pipeline=*/true);
  ...
  CHECK_EQ(scope.get(), GetScopeRoot(self, update_sref, /*require_stage_pipeline=*/false).get())
