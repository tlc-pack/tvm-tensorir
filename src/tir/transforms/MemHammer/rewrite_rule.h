...
class RewriteRule {
 public:
   ...
   */
  virtual Stmt Rewrite(const Stmt& stmt, const Map<String, ObjectRef>& constraints, Map<String, ObjectRef>*
                                                                                        output) const = 0;
  /*!
  ...
  inline Stmt Apply(const Stmt& stmt, const Map<String, ObjectRef>& constraints, Map<String,
                                                                                  ObjectRef>*
      output)
      const{
...
class CoalescedAccess : public RewriteRule{
 public:
  Stmt Rewrite(const Stmt &stmt, const Map<String, ObjectRef> &constraints, Map<String, ObjectRef>*
                                                                                output) const final;
  bool CanApply(const Stmt &stmt, const Map<String, ObjectRef> &constraints) const final{
    ...
    return (src_scope.rank==runtime::StorageRank::kGlobal && tgt_scope
.rank==runtime::StorageRank::kShared) || (src_scope.rank==runtime::StorageRank::kShared &&
...
class CreateLocalStage: public RewriteRule{
 public:
  ...
  bool CanApply(const Stmt &stmt, const Map<String, ObjectRef> &constraints) const final{
    String src = Downcast<String>(constraints["src_scope"]);
    String tgt = Downcast<String>(constraints["tgt_scope"]);
    PrimExpr has_local_stage = Downcast<PrimExpr>(constraints.Get("local_stage").value_or(Integer
                                                                                          (0)));
    runtime::StorageScope src_scope = runtime::StorageScope::Create(src);
    runtime::StorageScope tgt_scope = runtime::StorageScope::Create(tgt);
    return src_scope.rank==runtime::StorageRank::kGlobal && tgt_scope
                                                                      .rank==runtime::StorageRank::kShared && is_one(has_local_stage);
  }
};
