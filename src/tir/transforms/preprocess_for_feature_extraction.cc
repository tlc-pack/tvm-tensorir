...
arith::IntSet IntSetFromMinExtent1(const PrimExpr& min, const PrimExpr& extent) {
  return arith::IntSet::FromRange(Range::FromMinExtent(min, extent));
}

void NDIntSetUnionWith1(NDIntSet* lhs, const NDIntSet& rhs) {
...
Array<Range> NDIntSet2Region1(const NDIntSet& nd_int_set) {
  Integer one(1);
  Array<Range> result;
...
NDIntSet NDIntSetFromShape1(const Array<PrimExpr>& shape) {
  NDIntSet result;
  for (const PrimExpr& extent : shape) {
    result.push_back(IntSetFromMinExtent1(Integer(0), extent));
  }
  return result;
}

NDIntSet NDIntSetEmpty1(int ndim) {
  return std::vector<arith::IntSet>(ndim, arith::IntSet::Nothing());
}

bool IsThreadBound1(const For& loop) {
...
bool IsReduceTempBuffer1(const Buffer& buffer) {
  return StartsWith(buffer->name, "normal_reduce_temp") ||  //
         StartsWith(buffer->name, "reduce_temp");
}

PrimExpr BufferArea2(const Buffer& buffer) {
...
class ReductionTransformer1 : public StmtMutator {
 public:
  static Stmt Transform(const PrimFunc& f) { return ReductionTransformer1().VisitStmt(f->body); }
 ...
 */
class LCACollector1 : public StmtExprVisitor {
 public:
  static Map<Buffer, Optional<For>> Detect(const PrimFunc& func) {
    LCACollector1 detector;
...
class BufferAccessRewriter1 : public StmtExprMutator {
 public:
  ...
  static Stmt Rewrite(Stmt stmt, const FRewriteBufferAccess& f_rewrite) {
    BufferAccessRewriter1 rewriter(f_rewrite);
    return rewriter.VisitStmt(stmt);
  }

 private:
  explicit BufferAccessRewriter1(const FRewriteBufferAccess& f_rewrite) : f_rewrite_(f_rewrite) {}
  ...
  Stmt VisitStmt_(const BlockNode* _op) final {
    ...
    BlockReadWriteCollector collector(block->alloc_buffers);
    collector(block->body);
    op->reads = collector.reads();
    op->writes = collector.writes();
 ...
 */
class BufferAllocator1 : public StmtExprMutator {
 public:
  static Stmt Alloc(const PrimFunc& f) {
    Map<Buffer, Optional<For>> buffer_lca = LCACollector1::Detect(f);
    ...
    for (const auto& kv : f->buffer_map) {
      ...
      info.accessed_region = NDIntSetFromShape1(buffer->shape);
      info.alloc_site = NullOpt;
      info.region = NDIntSet2Region1(info.accessed_region);
      info.new_buffer = buffer;
      info.is_arg = true;
    }
    BufferAllocator1 alloc(std::move(buffer_info), std::move(loop_allocs));
    Stmt stmt = alloc.VisitStmt(f->body);
    stmt = BufferAccessRewriter1::Rewrite(
        /*stmt=*/std::move(stmt),
        /*f_rewrite=*/std::bind(&BufferAllocator1::RewriteBufferAccess,
                                &alloc,                 //
                                std::placeholders::_1,  //
                                std::placeholders::_2));
    return stmt;
 ...
 private:
  struct BufferInfo {
    ...
    explicit BufferInfo(int ndim, Optional<For> alloc_site)
        : accessed_region(NDIntSetEmpty1(ndim)),
  ...
  explicit BufferAllocator1(SMap<Buffer, BufferInfo> buffer_info,
                            SMap<Optional<For>, Array<Buffer>> loop_allocs)
      : block_nest_depth_(0),
    ...
    for (const BufferRegion& buffer_region : new_buffer_regions) {
      ...
      std::unordered_map<const VarNode*, arith::IntSet> dom_map;
      {
        ...
        for (const For& loop : this->ancestor_loops_) {
          ...
          if (need_relax || (buffer->scope == "shared" && IsThreadBound1(loop))) {
            dom_map[loop_var] = IntSetFromMinExtent1(loop->min, loop->extent);
          }
      ...
      NDIntSetUnionWith1(&info.accessed_region, int_set);
    }
 ...
 */
class BufferFlattener1 : public StmtExprMutator {
 public:
  static Stmt Flatten(const PrimFunc& f, bool lower_buffer = true) {
    BufferFlattener1 flattener;
 ...
 private:
  ...
  Stmt MakeAllocStmt(const Buffer& buffer, Stmt body, bool is_double_buffer) {
    ...
    PrimExpr area = BufferArea2(buffer);
...
class UnitLoopRemover : public StmtExprMutator {
 public:
  static Stmt removeUnitLoop(const PrimFunc& func) {
    UnitLoopRemover remover;
    return remover.VisitStmt(func->body);
  }

 private:
  Stmt VisitStmt_(const ForNode* node) {
    if (is_one(node->extent)) {
      return this->VisitStmt(node->body);
    } else {
...
PrimFunc PreprocessForFeatureExtraction(PrimFunc f) {
  ...
  fptr->body = ReductionTransformer1::Transform(f);
  // Step 2. Recalculate the buffer region
  fptr->body = BufferAllocator1::Alloc(f);
