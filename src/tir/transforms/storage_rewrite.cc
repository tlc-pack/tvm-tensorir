  ...
  void PrepareNewAlloc() {
    ...
    for (auto& kv : attach_map_) {
      ...
      for (size_t i = 0; i < vec.size(); ++i) {
        ...
        auto f_mul = [](PrimExpr a, PrimExpr b) { return a * b; };

        if (e->allocs.size() == 1) {
          // simply use the original allocation.
          PrimExpr sz = foldl(f_mul, make_const(DataType::Int(32), 1), e->allocs[0]->extents);
          sz = Align(sz);
          e->new_alloc =
          ...
          PrimExpr combo_size;
          for (const AllocateNode* op : e->allocs) {
            PrimExpr sz = foldl(f_mul, make_const(DataType::Int(32), 1), op->extents);
  ...
  Stmt VisitStmt_(const AllocateNode* op) final {
    ...
    if (t_vec.size() == 1 && t_vec[0].element_of() == op->dtype.element_of() &&
      ...
      if (me->base % factor == 0 && me->coeff % factor == 0) {
        ...
        return Allocate(op->buffer_var, t_vec[0], extents, op->condition, op->body);
      }
    }
    return stmt;
  }
