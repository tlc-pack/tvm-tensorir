...
/********** Multi-Level-Tiling-And-Fusion-With-TensorCore **********/

/*! \brief A rule that does multi-level tiling, and possibly cache_read/write */
class RuleMultiLevelTilingWithTensorCore {
 public:
  /*!
   * \brief Structure of the tiling.
   * Recommendation:
  ...
  struct State {
    ...
    explicit State(Schedule sch, BlockRV block_rv, Optional<BlockRV> write_cache = NullOpt,
                   bool write_cache_is_added = false, bool tensor_core_is_used = false,
                   Array<Array<LoopRV>> tiles = {})
        : sch(std::move(sch)),
          block_rv(std::move(block_rv)),
          write_cache(std::move(write_cache)),
  ...
  State TensorCoreCacheRead(State state) const {
    ...
    const Array<LoopRV>& r_tiles = state.tiles[r_idx.back()];
    // Insert cache_read block to the proper place
    ICHECK(!r_tiles.empty()) << "ValueError: Cannot find any reduction loop in the block";
  ...
  std::vector<State> AddWriteCache(State state) const {
    if (!can_cache_write) {
      // Case 0. Additional cache write stage is not allowed
      if (state.tensor_core_is_used) state = TensorCoreCacheWrite(state);
      return {std::move(state)};
    }
    std::vector<State> result;
    // Case 1. Do not add write cache, then fusion won't happen later either
    if (!must_cache_write) {
      // If tensorcore is applied, we give up inlining consumers
      if (state.tensor_core_is_used) {
        // Fork a new state and add the tensor core write stage
        State new_state(state);
        new_state.sch = Downcast<Schedule>(new_state.sch->Copy(state.sch->ForkSeed()));
        result.push_back(std::move(TensorCoreCacheWrite(new_state)));
      } else {
        // Otherwise, heck the only consumer of the block can be treated as write cache
        // If so, check if we can continuously inline a chain of only consumers for better locality
        Schedule sch = state.sch;
        BlockRV block_rv = state.block_rv;
        Array<BlockRV> consumer_chain = GetInlineableConsumerChain(sch, block_rv);
  ...
  std::vector<State> FuseWriteCache(State state) const {
    // If the only-consumer does not exist, or is not elementwise, then do not do fusion
    if (!state.write_cache.defined()) {
      if (state.tensor_core_is_used) state = TensorCoreWriteCacheFusion(state, r_idx.front() - 1);
      return {state};
    }
    std::vector<State> result;
    // Special case.
    ...
    if (!state.write_cache_is_added && !must_cache_write) {
      State new_state = state;
      new_state.sch = Downcast<Schedule>(new_state.sch->Copy(new_state.sch->ForkSeed()));
      if (new_state.tensor_core_is_used) {
        new_state = TensorCoreWriteCacheFusion(new_state, r_idx.front() - 1);
      }
      result.push_back(new_state);
    }
    Schedule sch = state.sch;
    ...
    for (int level : fusion_levels) {
      ...
      State new_state = state;
      new_state.sch = Downcast<Schedule>(state.sch->Copy(sch->ForkSeed()));
      if (new_state.tensor_core_is_used) new_state = TensorCoreWriteCacheFusion(new_state, level);
      new_state.sch->ReverseComputeAt(consumer, loop, true);
      result.push_back(new_state);
    }
    return result;
...
#define TVM_SEARCH_RULE_APPLY_SUB_RULE(SrcStates, FSubRule)                          \
  ...
  Array<Schedule> Apply(const SearchTask& task, const Schedule& sch,
    ...
    TVM_SEARCH_RULE_APPLY_SUB_RULE(states, DetectTensorCore);
    // Add write cache
    TVM_SEARCH_RULE_APPLY_SUB_RULE(states, AddWriteCache);
    // Do the multi-level tiling
    TVM_SEARCH_RULE_APPLY_SUB_RULE(states, DoTiling);
...
SearchRule MultiLevelTilingWithTensorCore(String structure, int max_innermost_factor,
  ...
  return SearchRule("multi_level_tiling_with_tensor_core", f_apply);
}
...
TVM_REGISTER_GLOBAL("meta_schedule.search_rule.MultiLevelTilingWithTensorCore")
    .set_body_typed(MultiLevelTilingWithTensorCore);
