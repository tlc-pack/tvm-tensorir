...
class RuleParallelizeVectorizeUnroll {
 public:
  ...
  Array<Schedule> Apply(const SearchTask& task, const Schedule& sch,
    ...
    if (HasAnyAnn(sch->GetSRef(root_block))) {
      return {sch};
    }
    // Parallelization
    if (max_jobs_per_core != -1) {
      ...
      sch->MarkBlock(root_block, tir::attr::auto_parallel_extent, max_extent);
    }
    // Vectorization
    if (max_vectorize_extent != -1) {
      sch->MarkBlock(root_block, tir::attr::auto_vectorize_extent, max_vectorize_extent);
    }
    // Unroll
    if (!unroll_max_steps.empty()) {
      ...
      if (unroll_explicit) {
        sch->MarkBlock(root_block, tir::attr::auto_unroll_explicit, max_step);
      } else {
        sch->MarkBlock(root_block, tir::attr::auto_unroll_implicit, max_step);
      }
    }
    return {sch};
  }
