...
class RewriteReductionBlockAutoMovementNode : public PostprocNode {
public:
 ...
 static constexpr const char* _type_key = "meta_schedule.RewriteReductionBlockAutoMovement";
 TVM_DECLARE_FINAL_OBJECT_INFO(RewriteReductionBlockAutoMovementNode, PostprocNode);
};

bool RewriteReductionBlockAutoMovementNode::Apply(const tir::Schedule& sch) {
 for (;;) {
   ...
   for (const auto& kv : results) {
     ...
     if (tir::HasAnn(block_sref, tir::attr::auto_tensor_core, "4")) {
       ...
       sch->Annotate(init_inner_block_rv[0], tir::attr::auto_tensor_core, String("4"));
     }
     ++rewritten;
   }
   if (rewritten == 0) {
...
Postproc Postproc::RewriteReductionBlockAutoMovement() {
 ObjectPtr<RewriteReductionBlockAutoMovementNode> n = make_object<RewriteReductionBlockAutoMovementNode>();
 return Postproc(n);
}

TVM_REGISTER_NODE_TYPE(RewriteReductionBlockAutoMovementNode);
TVM_REGISTER_GLOBAL("meta_schedule.PostprocRewriteReductionBlockAutoMovement")
   .set_body_typed(Postproc::RewriteReductionBlockAutoMovement);
