...
public:
 ...
 State TensorCoreLoad(State state) const {
   ...
   state.tensor_core_load_A = state.sch->ReadAt(r_tiles.back(), state.block_rv, 1, "wmma.matrix_a");
   state.tensor_core_load_B = state.sch->ReadAt(r_tiles.back(), state.block_rv, 2, "wmma.matrix_b");
...
inline std::vector<State> MultiLevelTilingMemHammerNode::AddWriteReuse(State state) const {
 ...
 for (int level : config.levels) {
   ...
   const LoopRV& loop_rv = new_state.tiles[level - 1].back();
   BlockRV write_cache = new_state.sch->WriteAt(/*loop_rv=*/loop_rv,/*block_rv=*/new_state.block_rv,
...
ScheduleRule ScheduleRule::MultiLevelTilingMemHammer(String structure, Optional<Array<String>> tile_binds,
                                           bool use_tensor_core,
                                           bool add_local_stage,
