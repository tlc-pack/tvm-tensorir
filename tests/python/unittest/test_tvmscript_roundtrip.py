...
def test_opaque_block():
    ...
    assert isinstance(rt_func.body.block, tir.stmt.Block)
    assert isinstance(rt_func.body.block.body, tir.stmt.For)
    assert isinstance(rt_func.body.block.body.body, tir.stmt.BlockRealize)
    assert isinstance(rt_func.body.block.body.body.block, tir.stmt.Block)
    block = rt_func.body.block.body.body.block
    assert len(block.iter_vars) == 0
