...
def sparse_dense_bsr(
    ...
    for i, j in tir.grid(M, N_blocks * bs_r):
        for block_offset, k in tir.grid(
            W_indptr[tir.floordiv(j, bs_r) + 1] - W_indptr[tir.floordiv(j, bs_r)], bs_c
        ):
            with tir.block(
                [
                    M,
                    N_blocks * bs_r,
                    tir.reduce_axis(0, W_indptr[j + 1] - W_indptr[j]),
                ...
                BSRmm[m, n] = (
                    BSRmm[m, n]
                    + W_data[offset + W_indptr[tir.floordiv(n, bs_r)], tir.floormod(n, bs_r), kk]
                    * X[m, bs_c * W_indices[offset + W_indptr[tir.floordiv(n, bs_r)]] + kk]
                )
...
def transpose(x: ty.handle, x_t: ty.handle) -> None:
    ...
    bs_c = tir.var("int32")
    tir.func_attr(
        {"flop_ct": 2 * M * num_blocks * bs_r * K, "tag": "sparse_dense_bsr", "tir.noalias": True}
    )
...
def sparse_dense_bsr_padded(
    ...
    warp_size: ty.int32 = tir.int32(32)
    ...
    with tir.block([], "root") as []:
        ...
        for ax0_outer in range(
            ...
            annotation={"loop_type": "blockIdx.x"},
        ):
                    ...
                    for ax0_inner in tir.thread_binding(0, 32, 'threadIdx.x'):
                        for ax1_inner in tir.thread_binding(0, 1, 'threadIdx.y'):
                            with tir.block([32, 1], "GPU_Thread") as [
                                ...
                                # since intermediate variables are not well supported, we will
                                # manually inline them
                                # m_index : ty.int32 = bx * warp_size + tx
                                # n_index : ty.int32 = by + ty
                                data_cache = tir.buffer_allocate(
                                    (warp_size, bs_r, bs_c),
                                    "float32",
                                    name="data_cache",
                                    scope="local",
                                ...
                                for ax4 in range(
                                    0, (W_indptr[by + ty + 1] - W_indptr[by + ty]) // warp_size
                                ):
                                    with tir.block(
                                        [(W_indptr[by + ty + 1] - W_indptr[by + ty]) // warp_size],
                                        "rowlength_bo",
                                    ) as [bb]:
                                        # cache indices
                                        # elem_idx : ty.int32 = W_indptr[by + ty] + bb * warp_size + tx
                                        w_indices_cache[tx] = W_indices[
                                            W_indptr[by + ty] + bb * warp_size + tx
                                        ]
                                        # cache dense matrix
                                        # each thread has a row
                                        for bi in range(0, warp_size):
                                            for x in range(
                                                0, bs_r, annotation={"loop_type": "unroll"}
                                            ):
                                                for z in range(
                                                    0, bs_c, annotation={"loop_type": "unroll"}
                                                ):
                                                    ...
                                                    data_cache[bi, x, z] = X_t[
                                                        w_indices_cache[bi] * bs_c + z,
                                                        (bx * 32 + tx) * bs_r + x,
                                                    ]
                                        # cache w_data
                                        for y in range(0, bs_r, annotation={"loop_type": "unroll"}):
                                            for z in range(
                                                ...
                                                w_data_cache[tx, y, z] = W_data[
                                                    W_indptr[by + ty] + bb * warp_size + tx, y, z
                                                ]
                                ...
                                for i in range(0, bs_r, annotation={"loop_type": "unroll"}):
                                    for j in range(0, bs_r, annotation={"loop_type": "unroll"}):
                                        with tir.block([bs_r, bs_r], "BSRmm") as [x, y]:
                                            if (
                                                bx * warp_size + tx < M_blocks
                                                and by + ty < N_blocks
                                            ):
                                                BSRmm[
                                                    (bx * 32 + tx) * bs_r + x, (by + ty) * bs_r + y
                                                ] = block[x, y]
...
def test_sparse_dense_padded():
    ...
    W_indices = te.placeholder(
        shape=W_sp_np.indices.shape, dtype=str(W_sp_np.indices.dtype), name="W_indices"
    )
    W_indptr = te.placeholder(
        shape=W_sp_np.indptr.shape, dtype=str(W_sp_np.indptr.dtype), name="W_indptr"
    )
    ...
    def check_device(device):
        ...
        with tvm.target.Target(device):
            ...
            evaluator = func.time_evaluator(func.entry_name, ctx, number=10)
            print(
                "transpose tir schedule: %f ms"
                % (evaluator(tvm.nd.array(X_np, ctx=ctx), X_t_tvm).mean * 1e3)
            )
