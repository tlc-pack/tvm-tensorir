...
extern "C" __global__ void __launch_bounds__(256) main_kernel0(half* __restrict__ A, half* __restrict__ B, float* __restrict__ C) {
  ...
  for (int ax0_0 = 0; ax0_0 < 2; ++ax0_0) {
    (void)nvcuda::wmma::load_matrix_sync(A_shared_wmma_matrix_a[ax0_0], ((half *)A_shared + ((((((int)threadIdx.y) >> 1) * 1280) + (ax0_0 * 640)))), 40);
  }
  for (int ax1_0 = 0; ax1_0 < 4; ++ax1_0) {
    (void)nvcuda::wmma::load_matrix_sync(B_shared_wmma_matrix_b[ax1_0], ((half *)B_shared + ((((((int)threadIdx.y) & 1) * 64) + (ax1_0 * 16)))), 136);
  }
  for (int i2_0_0 = 0; i2_0_0 < 31; ++i2_0_0) {
    for (int ax0_01 = 0; ax0_01 < 2; ++ax0_01) {
      (void)nvcuda::wmma::load_matrix_sync(A_shared_wmma_matrix_a[(ax0_01 + 2)], ((half *)A_shared + (((((((int)threadIdx.y) >> 1) * 1280) + (ax0_01 * 640)) + 16))), 40);
    }
    for (int ax1_01 = 0; ax1_01 < 4; ++ax1_01) {
      (void)nvcuda::wmma::load_matrix_sync(B_shared_wmma_matrix_b[(ax1_01 + 4)], ((half *)B_shared + (((((((int)threadIdx.y) & 1) * 64) + (ax1_01 * 16)) + 2176))), 136);
    }
  ...
  __syncthreads();
    for (int ax0_ax1_fused_02 = 0; ax0_ax1_fused_02 < 2; ++ax0_ax1_fused_02) {
      ((uint4*)(A_shared + (((((ax0_ax1_fused_02 * 2560) + (((int)threadIdx.y) * 320)) + ((((int)threadIdx.x) >> 2) * 40)) + ((((int)threadIdx.x) & 3) * 8)))))[0] = ((uint4*)((half*)A_shared_local + ((ax0_ax1_fused_02 * 8))))[0];
    }
    for (int ax0_ax1_fused_03 = 0; ax0_ax1_fused_03 < 2; ++ax0_ax1_fused_03) {
      ((uint4*)(B_shared + (((((ax0_ax1_fused_03 * 2176) + (((int)threadIdx.y) * 272)) + ((((int)threadIdx.x) >> 4) * 136)) + ((((int)threadIdx.x) & 15) * 8)))))[0] = ((uint4*)((half*)B_shared_local + ((ax0_ax1_fused_03 * 8))))[0];
    }
    __syncthreads();
    for (int ax0_02 = 0; ax0_02 < 2; ++ax0_02) {
      (void)nvcuda::wmma::load_matrix_sync(A_shared_wmma_matrix_a[ax0_02], ((half *)A_shared + ((((((int)threadIdx.y) >> 1) * 1280) + (ax0_02 * 640)))), 40);
    }
    for (int ax1_02 = 0; ax1_02 < 4; ++ax1_02) {
      (void)nvcuda::wmma::load_matrix_sync(B_shared_wmma_matrix_b[ax1_02], ((half *)B_shared + ((((((int)threadIdx.y) & 1) * 64) + (ax1_02 * 16)))), 136);
    }
  ...
  for (int ax0_03 = 0; ax0_03 < 2; ++ax0_03) {
    (void)nvcuda::wmma::load_matrix_sync(A_shared_wmma_matrix_a[(ax0_03 + 2)], ((half *)A_shared + (((((((int)threadIdx.y) >> 1) * 1280) + (ax0_03 * 640)) + 16))), 40);
  }
  for (int ax1_03 = 0; ax1_03 < 4; ++ax1_03) {
    (void)nvcuda::wmma::load_matrix_sync(B_shared_wmma_matrix_b[(ax1_03 + 4)], ((half *)B_shared + (((((((int)threadIdx.y) & 1) * 64) + (ax1_03 * 16)) + 2176))), 136);
  }
  ...
  for (int ax0_04 = 0; ax0_04 < 2; ++ax0_04) {
    for (int ax1_04 = 0; ax1_04 < 4; ++ax1_04) {
      (void)nvcuda::wmma::store_matrix_sync(((float *)C + (((((((((int)blockIdx.x) * 131072) + ((((int)threadIdx.y) >> 1) * 32768)) + (ax0_04 * 16384)) + (((int)blockIdx.y) * 128)) + ((((int)threadIdx.y) & 1) * 64)) + (ax1_04 * 16)))), C_wmma_accumulator[((ax0_04 * 4) + ax1_04)], 1024, nvcuda::wmma::mem_row_major);
    }
  }
}
