...
def lower_tvm_mfma_sync(op):
    wmma_D1, index_d, num_groups_d, wmma_A1, index_a, num_groups_a, wmma_B1, index_b, num_groups_b, wmma_C1, index_c, num_groups_c = op.args
    tx = tir.IterVar((0, 64), 'tx', tir.IterVar.ThreadIndex, 'threadIdx.x')
    return tir.AttrStmt(tx, 'thread_extent', 64, 
    tir.Store(
        wmma_D1,
        tir.call_llvm_pure_intrin(
            ...
            tir.Load(
                "float16x4",
                wmma_B1,
                tir.Ramp(tx * num_groups_b * 4 + index_b * 4, 1, 4),
                tir.Broadcast(True, 4),
                ),
            tir.Load(
                "float16x4",
                wmma_A1,
                tir.Ramp(tx * num_groups_a * 4 + index_a * 4, 1, 4),
                tir.Broadcast(True, 4),
                ),
            tir.Load(
                "float32x4",
                wmma_C1,
                tir.Ramp(tx * num_groups_c * 4 + index_c * 4, 1, 4),
                tir.Broadcast(True, 4),
                ),
            0,
            0,
        ...
        tir.Ramp(tx * num_groups_d * 4 + index_d * 4, 1, 4),
        tir.Broadcast(True, 4)
        ))

#register_intrin_lowering("tir.tvm_mfma_sync", target="default", f=lower_tvm_mfma_sync)
register_op_attr("tir.tvm_mfma_sync", "FLowerLogicalIntrin", lower_tvm_mfma_sync);
...
# def lower_logical_mma_sync(call):
#     return tir.call_llvm_intrin(call
