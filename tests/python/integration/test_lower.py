...
def tensorcore_gemm(a: ty.handle, b: ty.handle, c: ty.handle) -> None:
    ...
    C = tir.match_buffer(c, [1024, 1024], "float16")
    ...
    for blockIdx_x in tir.thread_binding(0, 16, "blockIdx.x"):
        for blockIdx_y in tir.thread_binding(0, 8, "blockIdx.y"):
            with tir.block([16, 8]) as [bx, by]:
                ...
                wmma_C = tir.alloc_buffer([1024, 1024], "float16", scope="wmma.accumulator")
                                ...
                                for i0, j0 in tir.grid(1, 4):
                                    for j1 in tir.vectorized(0, 4):
                                        with tir.block([1024, 1024]) as [vi, vj]:
                                            ...
                                            tir.bind(vj, ko * 32 + ty * 16 + j0 * 4 + j1)
                            ...
                            for ki in range(0, 2):
                                for i in range(0, 2):
                                    with tir.block([64, 64]) as [vi, vk]:
                                        ...
                                        tir.bind(vk, ko * 2 + ki)
                                        tir.reads(
                                            shared_A[vi * 16 : vi * 16 + 16, vk * 16 : vk * 16 + 16]
                                        )
                                        tir.writes(
                                ...
                                for j in range(0, 4):
                                    with tir.block([64, 64]) as [vj, vk]:
                                        ...
                                        tir.bind(vk, ko * 2 + ki)
                                        tir.reads(
                                            shared_B[vj * 16 : vj * 16 + 16, vk * 16 : vk * 16 + 16]
                                        )
                                        tir.writes(
...
if __name__ == "__main__":
    func = tvm.lower(tensorcore_gemm)
