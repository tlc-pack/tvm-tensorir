...
class Module1:
    def mmult(A: ty.handle, B: ty.handle, C: ty.handle) -> None:
        ...
        for x in range(0, 32, "parallel"):
            for y in range(0, 1024):
                for z in range(0, 32, "vectorized"):
                    packedB[x, y, z] = B_1[y, ((x*32) + z)]
        tir.attr(C_1, "realize_scope", "")
        tir.realize(C_1[0:1024, 0:1024])
        for x_outer in range(0, 32, "parallel"):
            for y_outer in range(0, 32):
                tir.attr(C_global, "realize_scope", "global")
                tir.realize(C_global[(x_outer*32):((x_outer*32) + 32), (y_outer*32):((y_outer*32) + 32)])
                for x_c_init in range(0, 32):
                    for y_c_init in range(0, 32, "vectorized"):
                        C_global[(x_c_init + (x_outer*32)), (y_c_init + (y_outer*32))] = tir.float32(0)
                for k_outer in range(0, 256):
                    for x_c in range(0, 32):
                        for k_inner in range(0, 4, "unroll"):
                            for y_c in range(0, 32, "vectorized"):
                ...
                for x_inner in range(0, 32):
                    for y_inner in range(0, 32):
...
def opt_conv_tensorcore_normalize(A: ty.handle, W: ty.handle, Conv: ty.handle) -> None:
    ...
    for n_c_init in range(0, 2):
        for o_c_init in range(0, 4):
    ...
    for ic_outer in range(0, 8):
        for kh in range(0, 3):
            ...
            for ax2 in range(0, 3):
                for ax3 in range(0, 2):
                    for ax4_ax5_fused_outer in range(0, 8):
            ...
            for ax1 in range(0, 3):
                for ax2_1 in range(0, 2):
                    tir.attr(tir.iter_var(threadIdx_x, None, "ThreadIndex", "threadIdx.x"), "thread_extent", 32)
                    for ax4_ax5_fused_inner in range(0, 8, "vectorized"):
    ...
    for n_inner in range(0, 2):
        for o_inner in range(0, 4):
